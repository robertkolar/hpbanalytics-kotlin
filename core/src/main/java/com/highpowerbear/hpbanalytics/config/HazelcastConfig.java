...
public class HazelcastConfig {
    ...
    public Map<String, ExchangeRateDTO> exchangeRateMap() {
        return hazelcastInstance().getMap(HanSettings.HAZELCAST_EXCHANGE_RATE_MAP_NAME);
    }

    @Bean
    @DependsOn("hazelcastInstance")
    public BlockingQueue<ExecutionDTO> executionQueue() {
        return hazelcastInstance().getQueue(HanSettings.HAZELCAST_EXECUTION_QUEUE_NAME);
    }

    @Bean
    @DependsOn("hazelcastInstance")
    public Map<String, List<Statistics>> statisticsMap() {
        return hazelcastInstance().getMap(HanSettings.HAZELCAST_STATISTICS_MAP_NAME);
    }

    @Bean
    @DependsOn("hazelcastInstance")
    public Map<String, List<Statistics>> currentStatisticsMap() {
        return hazelcastInstance().getMap(HanSettings.HAZELCAST_CURRENT_STATISTICS_MAP_NAME);
    }
